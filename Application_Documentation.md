...
│  │                         [User, Authority, Admin Dashboards]                 │   │
│  └─────────────────────────────────┬───────────────────────────────────────────┘   │
└────────────────────────────────────┼───────────────────────────────────────────────┘
                                     │ REST API (HTTP/JSON)
                                     │
...
│  │      Port: 8081            │   │     Port: 8080             │                   │
│  │                            │   │                            │                   │
│  │  • User Registration       │   │  • FIR Management          │                   │
│  │  • JWT Authentication      │◄──┤  • Missing Person Reports  │                   │
│  │  • Authority Management    │   │  • Case Tracking           │                   │
│  │  • Token Validation        │──►│  • Analytics               │                   │
│  └─────────────┬──────────────┘   └─────────────┬──────────────┘                   │
│                │  ▲                             │  │                               │
│                │  │ Feign Client                │  │ Async HTTP                    │
│                │  │ (Service Discovery)         │  │ (WebClient)                   │
│                │  │                             │  │                               │
│                │  └─────────────────────────────┘  │                               │
│                │                                   │                               │
│   ┌────────────┼───────────────────────────────────┼──────────────────────────┐    │
...
│                                                                                    │
│  ┌────────────────────────────┐   ┌────────────────────────────┐                   │
│  │      EMAIL SERVICE         │   │     LOGGING SERVICE        │                   │
│  │      (Node.js/Express)     │   │     (.NET/ASP.NET Core)    │                   │
│  │      Port: 3000            │   │     Port: 5000             │                   │
│  │                            │   │                            │                   │
│  │  • FIR Filed Notifications │   │  • Centralized Logging     │                   │
│  │  • Status Updates          │   │  • Structured Logs         │                   │
│  │  • Retry Queue             │   │  • Rolling File Storage    │                   │
│  │  • Mailtrap Integration    │   │  • Serilog Integration     │                   │
│  └────────────────────────────┘   └─────────────┬──────────────┘                   │
│                                                 │                                  │
└─────────────────────────────────────────────────┼──────────────────────────────────┘
                                                  │ File I/O
...
│                                                                                    │
│  ┌────────────────────────────┐   ┌────────────────────────────┐   ┌────────────┐  │
│  │      AUTH DATABASE         │   │    BACKEND DATABASE        │   │   LOGS     │  │
│  │      (MySQL 8.0)           │   │    (MySQL 8.0)             │   │  (Files)   │  │
│  │      Port: 3307            │   │    Port: 3306              │   │            │  │
│  │                            │   │                            │   │  /logs/    │  │
│  │  • users                   │   │  • firs                    │   │  ├─ user/  │  │
│  │  • authorities             │   │  • missing_persons         │   │  ├─ admin/ │  │
│  │  • admins                  │   │  • updates                 │   │  └─ fir/   │  │
│  └────────────────────────────┘   └────────────────────────────┘   └────────────┘  │
│         ▲                                  ▲                                       │
